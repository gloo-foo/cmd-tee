...
func TestTee_PartialOpenUnwind(t *testing.T) {
	...
	if !errors.Is(err, errBoom) {
		t.Fatalf("got %v, want the wrapped open cause errBoom", err)
	}
}
...
// TestTee_WriteError proves a destination write failure surfaces ErrWrite and
// still matches the underlying cause under errors.Is.
func TestTee_WriteError(t *testing.T) {
	...
	if !errors.Is(err, errBoom) {
		t.Fatalf("got %v, want the wrapped write cause errBoom", err)
	}
}
